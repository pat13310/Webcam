...
class QSettingMedia(QMainWindow):
    ...
    def __init__(self, conf: ConfigApp):
        ...
        self.conf = conf
        self.camera = WebCamera(0)
        self.camera2 = WebCamera(1)
        self.camera3 = WebCamera(2)
        self.camera4 = WebCamera(3)
        self.camera5 = WebCamera(self.conf.get_url("ip1"))
        self.camera6 = WebCamera(self.conf.get_url("ip2"))
        self.camera7 = WebCamera(self.conf.get_url("ip3"))
        self.camera8 = WebCamera(self.conf.get_url("ip4"))
    ...
    def on_start_camera(self):
        self.start_camera(self.camera, "webcam1")

    def on_start_camera2(self):
        self.start_camera(self.camera2, "webcam2")

    def on_start_camera3(self):
        self.start_camera(self.camera3, "webcam3")

    def on_start_camera4(self):
        self.start_camera(self.camera4, "webcam4")
    ...
    def start_camera(self, camera,infos):
        if camera.state=="stop":
            if camera.start():
                self.ui.lbl_infos.setText(f"{infos} disponible")
            else:
                self.ui.lbl_infos.setText(f"{infos} introuvable")
        else:
            self.camera.stop()

    def start_camera_IP(self, camera,infos):
        if camera.state=="stop":
           camera.startIP()
        else:
            self.camera.stop()
    ...
    def viewCam(self):
        if self.camera.is_start():
            ...
            qImg = qImg.scaledToHeight(175)
            self.ui.screen.setPixmap(QPixmap.fromImage(qImg))

        if self.camera2.is_start():
            trame2=self.camera2.update()
            qImg2 = QImage(trame2.image.data, trame2.width, trame2.height, trame2.step, QImage.Format_RGB888)
            qImg2=qImg2.scaledToHeight(175)
            self.ui.screen_2.setPixmap(QPixmap.fromImage(qImg2))

        if self.camera5.is_start():
            trame2 = self.camera5.update()
            qImg2 = QImage(trame2.image.data, trame2.width, trame2.height, trame2.step, QImage.Format_RGB888)
            qImg2 = qImg2.scaledToHeight(175)
            self.ui.screen_3.setPixmap(QPixmap.fromImage(qImg2))

        if self.camera6.is_start():
            trame2 = self.camera6.update()
            qImg2 = QImage(trame2.image.data, trame2.width, trame2.height, trame2.step, QImage.Format_RGB888)
            qImg2 = qImg2.scaledToHeight(175)
            self.ui.screen_4.setPixmap(QPixmap.fromImage(qImg2))

    def auto_detect(self):
        self.camera.start()
        res = self.camera.detect_resolution()
        self.camera.properties()
        self.camera.stop()
        self.ui.combo_res.clear()
        for w, h in res.items():
            st = w + "x" + h
